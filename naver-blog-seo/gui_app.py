...
class NaverBlogApp(tk.Tk):
    ...
    def _build(self) -> None:
        ...
        ttk.Label(ai, text="글·이미지 모두 OpenAI로 생성 (정보성 글 + GPT Image)").grid(
        ...
        self.phrases_var = tk.StringVar(value="")
        ...
        ttk.Button(btns, text="네이버 로그인(캡차)", command=lambda: self.start_job("login")).pack(
            side="left", padx=4
        )
    ...
    def save_settings(self) -> bool:
        ...
        save_config_yaml(
            ...
            post_times=times or ["09:30", "12:30", "15:30", "19:00"],
    ...
    def _run(self, mode: str) -> None:
        try:
            if mode == "login":
                ...
                self.log("로그인: 쿠키 우선 → 없으면 1회 캡차 후 쿠키 저장")
                ...
                try:
                    ...
                    if is_logged_in(driver):
                        n = save_cookies(driver, cookie_path(cfg.root))
                        self.log(f"로그인 OK — 쿠키 {n}개 저장. 이후엔 캡차 없이 재사용됩니다.")
                    else:
                        self.log("로그인 실패")
                finally:
                    driver.quit()
