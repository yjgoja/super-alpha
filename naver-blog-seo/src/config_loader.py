...
class AppConfig:
    ...
    body_image_model: str = "gpt-image-1-mini"
    body_image_quality: str = "low"
    min_chars: int = 2400
    ...
    *,
    ...
    data = {
        ...
        "openai": {
            ...
            "body_image_model": body_image_model,
            "body_image_quality": body_image_quality,
        },
        "content": {
            "min_sections": 7,
            "min_chars": min_chars,
...
def load_config(
    ...
    *,
    ...
    # 필수 문구는 선택사항 (빈 목록 허용). '이거다' 류는 기본 사용 금지.
    phrases = [
        str(p).strip()
        for p in (raw.get("required_phrases") or [])
        if str(p).strip() and str(p).strip() not in {"이거다", "이거다!"}
    ]

    openai_cfg = dict(raw.get("openai") or {})
    content_cfg = dict(raw.get("content") or {})
    footer_link = str(raw.get("footer_link") or FOOTER_LINK).strip() or FOOTER_LINK
    image_quality = str(openai_cfg.get("image_quality") or "medium").strip().lower()
    if image_quality not in {"low", "medium", "high", "auto"}:
        image_quality = "medium"
    body_image_quality = str(openai_cfg.get("body_image_quality") or "low").strip().lower()
    if body_image_quality not in {"low", "medium", "high", "auto"}:
        body_image_quality = "low"
    min_chars = int(content_cfg.get("min_chars") or 2400)
    ...
    return AppConfig(
        ...
        post_times=list(raw.get("post_times") or ["09:30", "12:30", "15:30", "19:00"]),
        keywords=keywords,
        required_phrases=phrases,
        body_image_count=int(raw.get("body_image_count", 8)),
        footer_link=footer_link,
        text_model=str(openai_cfg.get("text_model") or "gpt-4o"),
        image_model=str(openai_cfg.get("image_model") or "gpt-image-2"),
        image_quality=image_quality,
        body_image_model=str(openai_cfg.get("body_image_model") or "gpt-image-1-mini"),
        body_image_quality=body_image_quality,
        min_chars=min_chars,
        content=content_cfg,
